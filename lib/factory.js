...
module.exports = {
  ...
  createCore: function(opts) {
    ...
    function createWhenLikeFunction(keyword, opts) {
      opts = opts || {}
      return destructHashFor(keyword, function(assignTo, fn) {
        // When(fn)
        if (!assignTo) {
        ...
        // When(result, fn(done))
        if (opts.reportError)
          blockManager.addWhenToCurrent(resolveActionToResultOrThrow(fn, assignTo))
        else
    ...
    function resolveActionToResultOrThrow(fn, resultName) {
      return function(done) {
        var ctx = this
        if (isGeneratorFn(fn))
          fn = co.wrap(fn)
    ...
    return {
      ...
      When: createWhenLikeFunction('When', {reportError: false}),
      GivenI: createWhenLikeFunction('GivenI', {reportError: true}),
      LetI: createWhenLikeFunction('LetI', {reportError: true}),
      Then: Then,
      Invariant: Invariant,
      And: And,
