...
module.exports = {
  ...
  createCore: function(opts) {
    ...
    function Then() {
      ...
      var finalLabel = label || documentationify(thenFn)
      var finalFn = function(done) {
        ...
        function asyncifyExpectation(fn) {
          function warning(cb) {
            var keyword = fn === thenFn ? 'Then'
                  : ands.find(containsFn) ? 'And'
                  : 'Invariant'
            cb(new GoodHabitViolationError('async ' + keyword + ' is discouraged'))
          }
          function containsFn(f) { return f === fn }
          return fn.length === 0 ? async.asyncify(fn)
