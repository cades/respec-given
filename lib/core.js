...
function createCore(opts, blockManager) {
  ...
  function Then() {
    ...
    var thenFn = createFnFromArgs(arguments, { keyword: 'Then', meta: true })
      ...
      function runExpectations() {
        return sequencialExecute(invariants.concat(thenFn, ands), function(fn) {
          ...
          if (fn.call(ctx) === false)
            throw createNaturalAssertionError(fn, finalLabel, ctx)
        })
      }
  ...
  function createNaturalAssertionError(fn, finalLabel, ctx) {
    if (!fn._meta)
  ...
  function createFnFromArgs(args, opts) {
    opts = opts || {}
    var fn = findFirstThatIsFunction(args)
    if (opts.keyword)
      fn._keyword = opts.keyword
    if (opts.meta)
      fn.meta = findFirstThatIsPlainObject(args)
    return fn
  }
